        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class=" bg-white border-gray-200 px-3 pt-16 pb-6">
                        ...
                        <form method="POST" action="{{ route('subProduct.update',['subProduct' => $subProduct->id]) }}">
                            @method('PATCH')
                            @csrf
                            <div class="px-20">
                                ...
                                <select class="rounded-md shadow-sm w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" name="product">
                                  @foreach ($products as $product)
                                      <option value={{$product->id}} {{ $product->product_id == $product->id ? 'selected':'' }}>{{$product->designation}}</option>
                                  @endforeach
                                </select>
                                @error('product')
                                ...
                                <div class="mt-4 px-20">
                                    ...
                                    <x-input id="description" class="block mt-1 w-full" type="text" name="description" :value="old('description') ?? $subProduct->product_description" />
                                </div>
                            </div>
